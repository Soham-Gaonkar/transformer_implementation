...
def run_validation(model, validation_ds, tokenizer_src, tokenizer_tgt, max_len, device, print_msg, global_step, writer, num_examples=2):
    ...
    if writer:
        ...
        bleu = metric(
            [[p.split()] for p in predicted],
            [[e.split()] for e in expected]
        )
...
def train_model(config):
    device = "cuda" if torch.cuda.is_available() else "mps" if hasattr(torch, "has_mps") and torch.has_mps else "cpu"
